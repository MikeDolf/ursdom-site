...
for url, html in sorted(pages.items()):
    ...
    # Интерфейсные блоки, одинаковые на всех страницах по замыслу, как
    # шапка и подвал: калькулятор (в нём список из 47 городов и 11
    # материалов), полоса фото доставок, форма, блок связи, врезка
    # с призывом и гарантии. Пока они стояли внизу, в первые 6000 знаков
    # не попадали. После перестройки гео-страниц по схеме «цена,
    # калькулятор, фото, гарантии, форма, потом текст» они оказались
    # наверху и дали 520 пар выше порога при том же самом тексте
    # страниц. Без них максимум по гео-страницам 0,817 - то есть
    # сравниваются собственные тексты, ради которых проверка и есть.
    # Блоки продающего хвоста по образцу владельца помечены data-ui
    # прямо в макросах (гарантии, почему мы, цифры, шаги, форма, машины,
    # лента фото): так новый интерфейсный блок не нужно дописывать сюда
    # списком, а собственный текст страницы пометку не получит.
    body = re.sub(r'<(section|aside)[^>]*\bdata-ui\b[^>]*>.*?</\1>', " ", body, flags=re.S)
    for _pat in (r'<section class="d-calc".*?</section>',
                 r'<section class="d-fleet">.*?</section>',
                 r'<form\b.*?</form>',
                 r'<div class="d-callblock">.*?</div>\s*</div>',
                 r'<div class="d-midcta">.*?</div>',
                 r'<section class="d-dark-sec">.*?</section>'):
        body = re.sub(_pat, " ", body, flags=re.S)
