...
from site_config import SITE, ADVANTAGES, GUARANTEES, PAYMENT, PAYMENT_NOTE
...
# Готовые расчёты объёма. Раньше здесь стояла одна таблица на все
# материалы (заезд, подушка, дорога), и она же показывалась на страницах
# керамзита и гранитной крошки. Теперь у каждого материала своя таблица
# из data/tasks_calc.py, а по умолчанию берётся щебёночная оттуда же:
# в старой 108 кубов везли «4-5 полуприцепами» вместо шести рейсов.
from tasks_calc import task_rows, task_caption
CALC_ROWS = task_rows("shcheben")
...
def fit_range(base, lo, hi, extras, dot=False):
    """Подогнать длину title/description под диапазон [lo, hi] символов.

    Для генерируемых по матрице страниц базовая строка меняется в длину
    вместе с именем города (Реж - 5 букв, Каменск-Уральский - 19), и
    фиксированная приписка либо не долетает до lo на коротких именах,
    либо перелетает hi на длинных. extras - лесенка коротких, честных
    добавок возрастающей длины: перебираются все, отбираются влезающие
    в hi, из них берётся самая длинная - обычно она же самая близкая
    к диапазону снизу. Если ни одна не дотягивает до lo, отдаётся самая
    длинная из влезающих в hi: короче нормы лучше, чем длиннее её.
    dot=True добавляет точку в конце, если её ещё нет - для description,
    где предложение должно быть завершено.
    """
    candidates = []
    for extra in extras:
        cand = (base + extra).strip()
        if dot and not cand.endswith("."):
            cand += "."
        if len(cand) <= hi:
            candidates.append(cand)
    in_range = [c for c in candidates if len(c) >= lo]
    if in_range:
        return max(in_range, key=len)
    if candidates:
        return max(candidates, key=len)
    cut = (base + extras[-1])[:hi].rsplit(" ", 1)[0].rstrip(",.;: ")
    return cut + "." if dot else cut


def fit_range_multi(bases, lo, hi, extras, dot=False):
    """Как fit_range, но с несколькими вариантами базовой строки по
    убыванию длины - для названий материала, которые сами по себе
    растягиваются от «ПГС» до «Асфальтовая крошка». Первый вариант,
    для которого нашлась добавка внутри диапазона, и побеждает."""
    candidates = []
    for base in bases:
        for extra in extras:
            cand = (base + extra).strip()
    ...
    if candidates:
        return max(candidates, key=len)
    cut = bases[-1][:hi].rsplit(" ", 1)[0].rstrip(",.;: ")
    return cut + "." if dot else cut
...
def service_schema():
    """Service на главной раздела: сама доставка как услуга, отдельно
    от Product на конкретный материал. areaServed те же города, что
    и у LocalBusiness - разъезжаться с ним нечему, оба берут CITIES."""
    return {
        "@type": "Service",
        "serviceType": "Доставка нерудных строительных материалов",
        "name": SITE["tagline"],
        "provider": {"@id": DOMAIN + SITE["base"] + "#business"},
        "areaServed": [{"@type": "City", "name": n}
                       for n in ["Екатеринбург"] + [c["name"] for c in CITIES]],
        "url": DOMAIN + SITE["base"],
    }
...
PHOTOS = {
    ...
    "stati/skolko-shchebnya-v-kamaze": [
        ("samosval-karyer.jpg",
         ...
         "Гравийный щебень 20-40 крупным планом. Часть зёрен окатана, как галька, "
         "часть с гранями после дробления: это и отличает его от чистого гравия."),
    ],
    ...
    "fleet": [
        # Снимки с реальных доставок, присланные владельцем. На кадре
        # с синим самосвалом заретушированы телефон водителя на борту,
        # госномер и адресная табличка дома: это чужие данные.
        # Фракцию и породу в подписях не называем - по фото их не
        # установить, а неверная подпись хуже никакой.
        ("dostavki/samosval-vygruzka-krupnyy-shcheben.jpg",
         "Трёхосный самосвал с поднятым кузовом выгружает крупный щебень на участке у леса",
         "Выгрузка крупного щебня на участке. Большой самосвал ставим туда, где грунт "
         "держит машину с полным кузовом, и высыпаем материал одной кучей у места работ."),
        ("dostavki/samosval-shcheben-k-domu.jpg",
         "Малотоннажный синий самосвал с кузовом щебня на улице частного сектора",
         "Небольшой самосвал со щебнем у частного дома. Такая машина проходит по узким "
         "улицам и в ворота, куда большой самосвал не заедет."),
        ("dostavki/shcheben-v-kuzove.jpg",
         "Щебень в кузове самосвала крупным планом перед выгрузкой",
         "Щебень в кузове перед выгрузкой. При приёмке смотрите именно так: однородно ли "
         "зерно, нет ли пыли и комков глины."),
        ("dostavki/vygruzka-v-garazh.jpg",
         "Самосвал с поднятым кузовом высыпает щебень прямо в открытые ворота гаража",
         "Выгрузка прямо в гараж. Короткий кузов встаёт вплотную к воротам, и материал "
         "не приходится перекидывать лопатой с улицы."),
        ("dostavki/vygruzka-novostroyki.jpg",
         "Самосвал выгрузил кучу сыпучего материала мелкой фракции во дворе новостроек",
         "Выгрузка на площадке в новом жилом квартале. Точку выгрузки согласуем заранее, "
         "чтобы материал лёг там, где с ним будут работать."),
...
TRUCK_LABEL = {4: "малый самосвал", 10: "КамАЗ", 26: "большой самосвал"}


def _truck(vol):
    n, cap = calc_trips(vol)
    if n == 1:
        return TRUCK_LABEL[cap]
...
CITY_BANDS = [
    ...
    (150, (10, 15, 26),
     ...
     "за каждый рейс. Разумный минимум - десять кубов, оптимум - полный "
     "большой самосвал, до двадцати шести."),
    (10000, (15, 20, 26),
...
# ЩПС, бута и крошки строки в общем прайсе PER_CUBE нет, и блок «Цены
# на керамзит с доставкой» показывал щебень, песок и отсев, но не сам
# керамзит. Цена в карточке - нижняя из конфигурации страницы (та же,
# что в первом экране и в микроразметке), применение - из разделов
# «под задачу» этих же страниц. Скальный грунт в общем прайсе есть,
# поэтому ему своя карточка не нужна: он поднимается наверх через
# CATALOG_FIRST.
OWN_USE = {
    "keramzit": "Утепление пола и перекрытий, лёгкая стяжка, дренаж в клумбах",
    "graviy": "Дренаж, садовые дорожки, бетон до М250, ландшафтная отсыпка",
    "shchps": "Основание под плитку, заезды и дороги без отдельной расклинцовки",
    "butovyy-kamen": "Габионы, подпорные стенки, бутовый фундамент, берега",
    "granitnaya-kroshka": "Декоративная отсыпка клумб, дорожек и приствольных кругов",
    "asfaltovaya-kroshka": "Подъезды, дворы и стоянки, ремонт ям на проезде",
}


def own_shelf(slug, mat, mc):
    if slug not in OWN_USE:
        return []
    img = None
    if PHOTOS.get(slug):
        img = img_one("%s/%s" % (slug, PHOTOS[slug][0][0]))
    return [dict(name=mat["name"], price="от %s" % mc["low"], href=None, cell=34,
                 use=OWN_USE[slug], own=True, img=img,
                 alt=("%s, фотография материала" % mat["name"]) if img else None)]


# Насыпная плотность из блоков характеристик товарных страниц, т/м³.
# Нужна для вопроса «сколько тонн в машине»: весовая на выезде карьера
# и на въезде объекта считает тонны, а мы возим кубы.
DENSITY = {"pesok": (1.5, 1.6), "otsev": (1.4, 1.5), "pgs": (1.6, 1.7),
           "keramzit": (0.25, 0.45), "graviy": (1.4, 1.5), "shcheben": (1.4, 1.4)}
...
def tons_faq(mkey, rod):
    lo, hi = DENSITY[mkey]
    rng = lambda v: (_ru(v * lo) if lo == hi else "%s-%s" % (_ru(v * lo), _ru(v * hi)))
    dens = _ru(lo) if lo == hi else "%s-%s" % (_ru(lo), _ru(hi))
    return ("Сколько тонн %s в одной машине?" % rod,
            "Насыпная плотность %s около %s т/м³, поэтому КамАЗ на 10 кубов "
            "везёт %s т, двадцать кубов весят %s т. Считаем и возим мы в кубах, тонны нужны, "
            "чтобы сверить машину с весовой." % (rod, dens, rng(10), rng(20)))


def minvol_faq(rod, price_key, prep, km):
    """Минимальный объём с ценой куба в малой и в полной машине."""
    from calc import estimate
    p = FLOOR[price_key]
    a, b = [round(estimate(v, p, km)["total"] / v) for v in (4, 10)]
    return ("Какой минимальный объём заказа?",
            "Возим от 4 кубов. С доставкой %s куб %s в малом самосвале на 4 куба выходит около "
            "%d руб, в КамАЗе на 10 кубов около %d руб: рейс стоит одинаково, и чем "
            "больше кубов, тем меньше его доля в каждом." % (prep, rod, a, b))
...
ZONE_BANDS = [
    (0, 40, "Ближняя зона", "до 40 км",
     "Возим от 4 кубов, часто в день заявки."),
    ...
    (150, 10000, "Север и восток области", "от 150 км",
     "Разумно брать полный большой самосвал, до 26 кубов за рейс: дробить заказ на таком плече дороже всего."),
]
...
TRUCKS_SHOW = [
    dict(name="Малый самосвал", vol="до 4 м³", img=_fleet("samosval-shcheben-k-domu"),
         text="Узкие улицы частного сектора и ворота, куда большой не заедет. "
              "Рейс оплачивается целиком, поэтому четыре куба выгоднее брать сразу."),
    dict(name="КамАЗ", vol="до 10 м³", img=_fleet("samosval-karyer-shchebenka"),
         text="Ходовая машина: «КамАЗ щебня» или «КамАЗ песка» - это она. "
              "Нужен разворот и твёрдая площадка под выгрузку."),
    dict(name="Большой самосвал", vol="до 26 м³", img=_fleet("samosval-vygruzka-krupnyy-shcheben"),
         text="До двадцати шести кубов за рейс: на дальнем плече куб выходит "
...
SALES = dict(
    ...
    trust=[
        ...
        ("truck", "4-26 м³", "самосвалы под любой заезд"),
    ...
    why=[
        ...
        ("truck", "Машина под ваш заезд", "Малый самосвал на узкую улицу, КамАЗ до 10 кубов, большой до 26 кубов на дальнее плечо."),
        ("route", "Скажем, когда невыгодно", "На дальнем плече доставка бывает дороже материала, и мы говорим это до заказа."),
        ("chat", "Ответ за пять минут", "Пишите в MAX: отвечаем там круглосуточно."),
...
# Способы оплаты в общем контексте: раньше их передавали только четыре
# вызова из десятка, и на дресве, ЖБИ и бетоне блок «Как можно
# заплатить» выходил с одним заголовком.
BASE_CTX = dict(cfg=SITE, advantages=ADVANTAGES, guarantees=GUARANTEES, g=SALES,
                payment=PAYMENT, payment_note=PAYMENT_NOTE,
...
htmlp = env.get_template("hub.j2").render(
    **BASE_CTX, title="Доставка нерудных материалов в Екатеринбурге: щебень, песок",
    desc="Доставка щебня, песка, ПГС и отсева по Екатеринбургу и Свердловской области. Самосвалы от 4 до 26 кубов, оплата после выгрузки, честный объём по кузову.",
...
money_cfg = {
    ...
    "pesok": dict(hero_sub="Карьерный и речной песок с доставкой по " + SITE["region_dat"] + ". Под отсыпку, подушку фундамента, бетон и кладку. Самосвалы от 4 кубов, оплата после выгрузки.",
    ...
    "pgs": dict(hero_sub="Природная ПГС и обогащённая ОПГС с доставкой по " + SITE["region_dat"] + ". Под планировку территории, подсыпку оснований и обратную засыпку.",
                ...
                desc="ПГС и ОПГС с доставкой по Екатеринбургу и области под отсыпку и планировку. Самосвалы 4-26 м³, заявки круглосуточно, оплата после выгрузки, без предоплаты.",
...
for slug, mc in money_cfg.items():
    ...
    htmlp = env.get_template("money.j2").render(
        **_ctx, **mc, calc=_calc, has_calc=bool(_calc),
        ...
        own_calc_rows=task_rows(slug), own_calc_caption=task_caption(slug),
        cross=cross_for(slug), zones=zones_for(),
...
def geo_faq(c, mat_rod="щебня", mat_vin="щебень"):
    ...
    return [
        ...
        ("Какой минимальный объём заказа?",
         (c['min_note'] if c['min_note'].startswith("Возим от")
          else f"Возим от 4 кубов, дальше объём подбираем под задачу и заезд. {c['min_note']}")),
        ("Нужна ли предоплата?",
...
# Что смотреть при приёмке именно этого материала. Шаг «проверяете
# объём» был общим для всех, хотя у песка и у керамзита проверяют
# разное, и покупателю это полезнее, чем одинаковая фраза.
ACCEPT_CHECK = {
    "pesok": "У песка сожмите горсть: если комок мажет ладонь, в нём много глины.",
    "otsev": "В отсеве не должно быть крупного камня и комков земли.",
    "keramzit": "У керамзита гляньте на долю битых гранул и пыли: целая гранула круглая, с плотной коркой.",
    "graviy": "Чистый гравий не пачкает руки; налёт глины и песка на камне заметен сразу.",
    "pgs": "ПГС должна быть сыпучей, без крупных комков глины.",
}
...
def gen_mat_city(mkey, price_key, rod, vin, calc_slug):
    ...
    for i, cs in enumerate(cities):
        ...
        # Вопросы с ответом по числам этого города. Раньше первым шёл
        # «Сколько стоит доставка» с ответом «зависит от объёма», в ответе
        # про минимальный объём одна фраза повторялась дважды, а «Какой
        # отсев берут» дословно повторял раздел страницы.
        _g = geo_faq(place, rod, vin)
        cfaq = [trip_faq(rod, price_key, f["prep"], skm), _g[1],
                minvol_faq(rod, price_key, f["prep"], skm), _g[3]]
        if mkey in DENSITY:
            cfaq.append(tons_faq(mkey, rod))
        jl = graph(localbusiness(), bc_schema(crumb_items), faq_schema(cfaq),
        ...
        sections = [
            ...
            {"id": "chto", "h": "Какой %s берут %s" % (vin, f["loc"]),
             # Строка короткая и по смыслу повторяет абзац выше, но убирать
             # её нельзя: это единственное место, где у отсева и песка
             # одного города разный текст в первых экранах. Без неё пары
...
for _ci, _c in enumerate(CALC_PAGES):
    ...
    _faq = [
        ("Как рассчитать, сколько %s нужно на площадку?" % _name,
    ...
    htmlp = env.get_template("calcpage.j2").render(
        **BASE_CTX, **hero_ctx(_c["slug"]),
        canonical=DOMAIN + url, crumbs_html=crumbs(crumb_items), jsonld=jl,
        title=fit_range_multi([
                "Калькулятор %s онлайн: объём, вес, цена с доставкой" % _name,
        ...
        desc=fit_range_multi([
                ...
                "Калькулятор %s: объём по площадке с запасом на уплотнение, "
                "перевод кубов в тонны, цена с доставкой по %s"
                % (_name, SITE["region_dat"]),
                "Калькулятор %s: объём с запасом на уплотнение, перевод в тонны "
                "и цена с доставкой по %s" % (_name, SITE["region_dat"])],
                150, 160, GEO_DESC_PAD, dot=True),
        h1=h1,
...
htmlp = env.get_template("calchub.j2").render(
    **BASE_CTX, **hero_ctx("shcheben"),
    ...
    title="Калькуляторы нерудных материалов: объём, вес, цена",
    desc=("Калькуляторы щебня, песка, отсева, ПГС и других материалов: объём "
          "по площадке с запасом на уплотнение, перевод кубов в тонны, "
...
for slug, mc in MONEY_CFG_EXT.items():
    ...
    htmlp = env.get_template("money.j2").render(
        ...
        **buyers_for(slug), buy_sections=KUPIT_SECTIONS.get(slug, []),
        ...
        specs=mat.get("specs"), specs_head=mat.get("specs_head"),
        packs=mat.get("packs"), packs_head=mat.get("packs_head"),
        quick=mat.get("quick"), shelf_rows=own_shelf(slug, mat, mc) + catalog_for(slug),
        own_calc_rows=task_rows(slug), own_calc_caption=task_caption(slug),
        cross=cross_for(slug), zones=zones_for(),
...
def geo_city_faq(facts, mats, pl, dist, skm):
    ...
    q.insert(0, trip_faq("щебня", "Щебень 20-40", facts["prep"], skm,
...
for city_slug, mats in MATRIX.items():
    ...
    h_grunt = f"Грунты {facts['loc']} и что из этого следует"
    if old:
        p_plecho, p_kuda, p_grunt = old["block"], old["areas"], old["objects"]
        h_grunt = f"Что чаще всего заказывают {facts['loc']}"
        p_econ = old["use"]
        p_sroki = old["terms"]
    else:
...
_c_faq = [
 ["Где вы находитесь?",
  "База в Екатеринбурге, %s, и вторая площадка отгрузки в районе Верхней Пышмы. "
  "Машина идёт с той, что ближе к вам: на север по Серовскому тракту, в Тагил, "
  "Невьянск и Новоуральск, это Пышма, на остальные направления Екатеринбург. "
  "На дальнем плече часто грузим прямо на карьере ближе к объекту, и это дешевле." % SITE["street"]],
 ...
 ["Как быстро вы отвечаете?",
  "В MAX обычно в течение пяти минут, работаем круглосуточно "
 ...
 ["Какой минимальный заказ?",
  "От четырёх кубов одной машиной. Меньше тоже привезём, но рейс "
...
_c_sections = [
 ...
 {"id": "avtopark",
  "h": "Наш автопарк",
  "p": ["Возим самосвалами от 4 до 26 кубометров. Машина подбирается "
        "не по объёму заказа, а по заезду: там, где большой самосвал "
        "не развернётся, идут ходки КамАЗом или малым, и это обсуждается "
        "до выезда, а не у ворот."],
  "table": {
   "caption": "Чем возим и куда это проходит",
   "head": ["Машина", "Объём", "Где проходит"],
   "rows": [
    ["Малый самосвал", "до 4 м³", "Узкие улицы, дачные проезды, участки без разворота"],
    ["КамАЗ", "до 10 м³", "Обычный частный участок с нормальным заездом"],
    ["Большой самосвал", "до 26 м³", "Открытая площадка, объект, широкий заезд"],
...
htmlp = env.get_template("contacts.j2").render(
    ...
    **BASE_CTX, hero_img=img_one("fleet/baza-s-vozduha.jpg"), reviews=REVIEWS,
    ...
    desc='Контакты ООО «Щебень-Урал»: база в Екатеринбурге, ул. Толедова, 43, работаем круглосуточно без выходных. Честный объём по кузову, оплата после выгрузки.',
    h1="Контакты и как мы работаем",
    hero_sub=("База в Екатеринбурге, %s, вторая площадка у Верхней Пышмы. Возим по всей "
              "Свердловской области, объём проверяется по кузову до разгрузки, оплата после выгрузки."
              % SITE["street"]),
