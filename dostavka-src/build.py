...
def tons_faq(mkey, rod):
    ...
    return ("Сколько тонн %s в одной машине?" % rod,
            "Насыпная плотность %s около %s т/м³, поэтому самосвал на 10 кубов "
            "везёт %s т, на 20 кубов %s т. Считаем и возим мы в кубах, тонны нужны, "
...
def minvol_faq(rod, price_key, prep, km):
    ...
    from calc import estimate
    p = FLOOR[price_key]
    a, b = [round(estimate(v, p, km)["total"] / v) for v in (5, 10)]
    return ("Какой минимальный объём заказа?",
            "Возим от 5 кубов. С доставкой %s куб %s в машине на 5 кубов выходит около "
            "%d руб, в машине на 10 кубов около %d руб: рейс стоит одинаково, и чем "
    ...
    return [
        ...
        ("Какой минимальный объём заказа?",
         (c['min_note'] if c['min_note'].startswith("Возим от")
          else f"Возим от 5 кубов, дальше объём подбираем под задачу и заезд. {c['min_note']}")),
        ("Нужна ли предоплата?",
...
for city_slug, mats in MATRIX.items():
    ...
    desc = fit_range(f"Доставка {facts['prep']} и по округу: " + _names_str +
                     f". Плечо {dist}, самосвалы от 5 до 20 кубов, оплата после выгрузки",
                     150, 160, GEO_DESC_PAD, dot=True)
    hero = (f"Везём " + ", ".join(names) + f" {facts['prep']} самосвалами "
            f"от 5 до 20 кубов. Рейс {pl['tier']}, {dist}: {pl['term']}. "
