...
# Разбор выгрузки Букварикса дал ещё четыре расхождения того же рода:
# товар у нас есть, а называем мы его не тем словом, которым ищут.

# Раньше здесь стояла строка под поисковик: «щебень часто набирают как
# щебенка, это разговорное название той же фракции». Человеку она ничего
# не давала, а «щебёнка» к тому же не фракция. На её месте ответ на
# вопрос, который возникает у таблицы цен: во что обойдётся машина.
# Числа из фиксированных цен за машину по Екатеринбургу (LOTS в prices.py,
# таблица «Готовые объёмы с доставкой» на странице щебня), а не расчётом
# по километражу: по городу фиксированная цена ниже, и называть здесь
# другую значило бы спорить с товарной страницей.
def _nerud_note():
    from prices import LOTS
    def n(x):
        return int(x.replace(" ", ""))
    rows = [(int(v.split()[0]), n(p)) for v, p, _p2, _note in LOTS]
    per = [round(p / v) for v, p in rows]
    fmt = lambda x: f"{x:,}".replace(",", " ")
    return ("Машина щебня 20-40 с доставкой по Екатеринбургу стоит фиксированно: "
            + ", ".join("%d кубов %s руб" % (v, fmt(p)) for v, p in rows)
            + ". Куб с доставкой выходит от %s до %s руб в зависимости от машины."
            % (fmt(min(per)), fmt(max(per))))


NOTE_NERUD = _nerud_note()

NOTE_BORDYUR = ("Маркировка БР 100.30.15 по ГОСТ 6665 читается как размер в сантиметрах: "
                "это бордюр 1000×300×150 мм, он же «бордюр 1000х30х15». Соответственно "
                "БР 100.20.8 это 1000×200×80 мм. Обозначения разные, изделие одно.")

NOTE_BLOKI = ("Размеры блоков на сайте набраны знаком умножения, 600×300×200, "
              "а в поиске их набирают буквой х: 600х300х200, 600х400х300. "
              "Это один и тот же ходовой формат, разница только в раскладке.")
...
OBJECTIONS = [
 ("Минимальный объём", "Возим от 4 кубов одной машиной. Меньший объём тоже привезём, но за куб выйдет дороже: рейс оплачивается целиком."),
